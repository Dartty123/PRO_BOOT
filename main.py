...
login_manager.login_message = "Для куппівлі туру увійдіть до системи "
...
def global_data():
    with Session() as session:
        if current_user.is_authenticated:
            user = session.query(Tour).where(User.id == current_user.id).first()
            user_tours = user.tours
        else:
            user_tours = []

    return dict(departures=data.departures, user_tour=user_tours)
...
def login():
    ...
    if login_form.validate_on_submit():
        ...
        with Session() as session:
            ...
            if not user:
                flash("Такого користувача не існує.Зареєструйтесь")  
